...
const evaluator = new Function('document', 'chatMessages', `const golfDate=document.getElementById('golfDate'),golfEndDate=document.getElementById('golfEndDate'),golfPlayers=document.getElementById('golfPlayers'),golfBudget=document.getElementById('golfBudget'); let consultationRegion = ''; const updateStayLength = () => {};${source}\nreturn { extractConsultationConditions, consultationSummary };`);
const { extractConsultationConditions, consultationSummary } = evaluator(document, chatMessages);
